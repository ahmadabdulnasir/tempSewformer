...
class Backbone(BackboneBase):
    ...
    def __init__(self, name:str, 
        ...
        if in_channel != 3:
            ...
            new_layer.weight[:, :layer.in_channels, :, :].data[...] = weight

            #Copying the weights of the `copy_weights` channel of the old layer to the extra channels of the new layer
            for i in range(in_channel - layer.in_channels):
                channel = layer.in_channels + i
                new_layer.weight[:, channel:channel+1, :, :].data[...] = weight[:, copy_weights:copy_weights+1, : :]
...
def build_backbone(args):
    position_embedding = build_position_encoding(args["NN"])
    train_backbone = float(args["trainer"]["lr_backbone"]) > 0
    return_interm_layers = args["NN"]["masks"] 
    backbone = Backbone(args["NN"]["backbone"], 
                        args["NN"]["in_channel"],
                        train_backbone, 
